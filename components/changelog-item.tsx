...
export function ChangelogItem({ entry, isLast }: ChangelogItemProps) {
  ...
  return (
    <div className="relative pb-12 group last:pb-0">
      ...
      {!isLast && (
        <div
          className="absolute left-[11px] top-6 h-full w-[2px] bg-border/40 group-last:hidden"
          aria-hidden="true"
        />
      )}
      ...
      <div className="ml-10 space-y-4">
        <div className="flex flex-wrap items-center gap-3">
          <Badge
            variant="outline"
            className="font-mono text-accent-blue border-accent-blue/30 px-2.5 py-0.5 rounded-md"
          >
            v{entry.version}
          </Badge>
                  ...
                  <div key={idx} className="p-5 sm:p-6 space-y-3">
                    ...
                    <ul className="space-y-2.5">
                      {change.items.map((item, i) => (
                        <li
                          key={i}
                          className="flex items-start text-[15px] leading-normal text-foreground/90"
                        >
